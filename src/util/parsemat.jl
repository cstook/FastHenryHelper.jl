...
function parsefasthenrymat(io::IO)
  ...
  portnames = Array{AbstractString}(undef,numberofrows)
  ...
  frequencies = 0
  buffer = IOBuffer()
  impeadancematrix = Array{Complex{Float64}}(undef,numberofrows,numberofrows)
  while ~eof(io)
    line = readline(io)
    frequencies+=1
  ...
  resultmatrix = Array{Complex{Float64}}(undef,numberofrows,numberofrows,frequencies)
